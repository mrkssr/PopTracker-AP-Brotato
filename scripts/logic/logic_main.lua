...
Brotato_location = {}
Brotato_location.__index = Brotato_location
...
function Brotato_location.new(name)
    local self = setmetatable({}, Brotato_location)
    if name then
...
function Brotato_location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        exit = Brotato_location.new(exit)
    end
    if rule == nil then
        rule = always
    end
...
function Brotato_location:connect_two_ways(exit, rule)
...
function Brotato_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
function Brotato_location:connect_two_ways_entrance(name, exit, rule)
...
-- acts as a shortcut for 2 connect_one_way_entrance-calls
function Brotato_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
function Brotato_location:accessibility()
...
-- 
function Brotato_location:discover(accessibility, keys)
...
entry_point = Brotato_location.new("entry_point")
